...
class TelegramService:
    ...
    def _send_photo(self, image_url: str, caption: str) -> bool:
        try:
            response = self.session.post(f"{self._base_url}/sendPhoto", json={"chat_id": self.settings.telegram_chat_id, "photo": image_url, "caption": caption, "parse_mode": "HTML"}, timeout=30)
    ...
    def _send_message(self, text: str) -> bool:
        try:
            response = self.session.post(f"{self._base_url}/sendMessage", json={"chat_id": self.settings.telegram_chat_id, "text": text, "parse_mode": "HTML", "disable_web_page_preview": False}, timeout=30)
            response.raise_for_status()
            return True
        except requests.RequestException:
